...
void insertSortU(int k, NumericVector values, IntegerVector ind, int n, int left){
  //insert k-th value (we know it is new) into vector values keeping values sorted
  //left the mid calculated in findDupU1
  //this function is used together with findDupU1 to do the unique sort
...
void testU (NumericVector values){
  values(0) = 0;
}

int findDupU(NumericVector values, double value, int n){
...
int findDupU1(NumericVector values, IntegerVector ind, int k, int n){
...
double kernelDensU(double x, double left, double right, double p){
...
IntegerVector findLowerBoundU1(arma::colvec resid, int ind, IntegerVector s){
...
IntegerVector findLowerBoundU2(arma::colvec resid, int ind, IntegerVector s){
...
double f1U(double p, double sigma1, double sigma2, double x){
...
double f2U(double p, double sigma1, double sigma2, double x){
  //help to draw the regression coefficients
  if (x > 0){
    return (1.0 - p) / sigma2;
  }
...
double trunGammaLeftU(double a, double b, double t){
...
double proposalFcnU(NumericVector w, double a, int nc){
...
double trunGammaRightU(double a, double b, double t){
...
double trunNormalLRU (double left, double right, double mu, double sigma){
...
//'
//[[Rcpp::export]]
List bqrDPMU (NumericMatrix Xr, NumericVector Yr, double p, int nsim, NumericVector initCoef, int burn = 5000, bool plotDensity = false, double tol=0.0000001) {
  ...
  n2 = 1;
  for (i=1; i<size; i++){
    s1(i) = findDupU(sigmaStar1, sigma1(i), n1);
    if (s1(i) == -1){
    ...
    s2(i) = findDupU(sigmaStar2, sigma2(i), n2);
    if (s2(i) == -1){
  ...
  for (sim=0; sim<nsim; sim++){
    ...
    int noInfo = 0;
    for (j=0; j<n1; j++){
      IntegerVector v = findLowerBoundU1(resid, j, s1);
      if (v(1) == 0){
        noInfo++;
        ...
        if (temp > 0.95){
          ...
          randSample = 1.0 / trunGammaRightU(c1 + v(1), 1.0 / d1, -1.0 / resid(v(0)));
        }
    ...
    noInfo = 0;
    for (j=0; j<n2; j++){
      IntegerVector v = findLowerBoundU2(resid, j, s2);
      if (v(1) == 0){
        noInfo++;
        ...
        if (temp > 0.95){
          ...
          randSample = 1.0 / trunGammaRightU(c2 + v(1), 1.0 / d2, 1.0 / resid(v(0)));
        }
        ////Rcout << randSample << "\t" << resid(v(0)) << "\n";
    ...
    for (i=0; i<size; i++){
      ...
      if (resid(i) < sigma2(i)){
        if (resid(i) < 0){
          ...
          if (temp > 0.95){
            ...
            randSample = 1.0 / trunGammaRightU(c1 + 1, 1.0 / d1, -1.0 / resid(i));
          }
        ...
        for (j=0; j<n1; j++){
          if (j != s1(i)){
            prob(j) = clusterSize1(j) * kernelDensU(resid(i), sigmaStar1(j), sigma2(i), p);
            ////Rcout << resid(i) << "\t" << -sigmaStar1(j) << "\t" <<sigma2(i)<<"\t" << kernelDensU(resid(i), sigmaStar1(j), sigma2(i), p) << "\n";
          }
          else {
            prob(j) = (clusterSize1(j) - 1.0) * kernelDensU(resid(i), sigmaStar1(j), sigma2(i), p);
          }
        }
        ////Rcout <<"error\t"<< 2;
        //normalize
      ...
      //sampling from the mixture of continuous and dicrete distribution with probability available
        ...
        if (temp <= probCont){
          sigma1(i) = randSample;
          int ind = findDupU(sigmaStar1, sigma1(i), n1);
          if (ind == -1) {
            sigmaStar1(n1) = sigma1(i);
            ////Rcout<<"flag1!";
            clusterSize1(n1) = 1;
      ...
      if (resid(i) + sigma1(i) > 0){
        if (resid(i) > 0){
          ...
          if (temp > 0.95){
            ...
            randSample = 1.0 / trunGammaRightU(c2 + 1.0, 1.0 / d2, 1.0 / resid(i));
          }
          ////Rcout << randSample << "\t" << resid(i) << "\n";
        ...
        for (j=0; j<n2; j++){
          if (j != s2(i)){
            prob(j) = clusterSize2(j) * kernelDensU(resid(i), sigma1(i), sigmaStar2(j), p);
          }
          else {
            prob(j) = (clusterSize2(j) - 1) * kernelDensU(resid(i), sigma1(i), sigmaStar2(j), p);
          }
        }
        //normalize
        temp = probCont;
        ...
        if (temp <= probCont){
          sigma2(i) = randSample;
          int ind = findDupU(sigmaStar2, sigma2(i), n2);
          if (ind == -1) {
            sigmaStar2(n2) = sigma2(i);
            clusterSize2(n2) = 1;
            clusterSize2(s2(i))--;
    ...
    //for (int k=ncov-1; k>-1; k--){
      ...
      for (i=0; i<size; i++){
        if (X(i,k) < 0){
          temp = u(i) - sigma2(i) / X(i,k);

          if (temp < upperBound){
            upperBound = temp;
          }
          temp = u(i) + sigma1(i) / X(i,k);
          if (temp > lowerBound){
            lowerBound = temp;
          }
      ...
      if (tempInd == 0){
        beta(k) = trunNormalLRU(lowerBound, upperBound, mu(k), sdDiag(k));
      }
      else {
        ...
        for (i=0; i<tempInd; i++){
          tempInd2 = findDupU1(u, tempVec1, tempVec(i), tempInd1);
          if (tempInd2 < 0){
            insertSortU(tempVec(i), u, tempVec1, tempInd1, -(tempInd2+1));
            tempInd1++;
          }
        }
        ...
        for (i=0; i<tempInd1-1; i++){
          ...
          for (j=0; j<tempInd; j++){
            if (u(tempVec(j)) <= u(tempVec1(i)) && X(tempVec(j), k) != 0){
              prob(i) += log(f1U(p, sigma1(tempVec(j)), sigma2(tempVec(j)), X(tempVec(j), k)));
            }
            if (u(tempVec(j)) >= u(tempVec1(i+1)) && X(tempVec(j), k) != 0){
              prob(i) += log(f2U(p, sigma1(tempVec(j)), sigma2(tempVec(j)), X(tempVec(j), k)));
            }
          }
          prob(i) = exp(prob(i));
        }
        ...
        for (j=0; j<tempInd; j++){
          if (X(tempVec(j), k) != 0) {
            probCont += log(f2U(p, sigma1(tempVec(j)), sigma2(tempVec(j)), X(tempVec(j), k)));
          }
        }
        ...
        for (j=0; j<tempInd; j++){
          if (X(tempVec(j), k) != 0) {
            prob(tempInd1-1) += log(f1U(p, sigma1(tempVec(j)), sigma2(tempVec(j)), X(tempVec(j), k)));
          }
        }
        ...
        temp = R::runif(0.0, 1.0);
        if (temp <= probCont){
          beta(k) = trunNormalLRU(lowerBound, u(tempVec1(0)), mu(k), sdDiag(k));
        }
        else if (temp + prob(tempInd1-1) > 1){
          beta(k) = trunNormalLRU(u(tempVec1(tempInd1-1)), upperBound, mu(k), sdDiag(k));
        }
        else{
          double cumProb = probCont;
          for (j=0; j<tempInd1-1; j++){
            if (temp > cumProb && temp <= cumProb + prob(j)){
              beta(k) = trunNormalLRU(u(tempVec1(j)), u(tempVec1(j+1)), mu(k), sdDiag(k));
              break;
            }
            cumProb += prob(j);
          }
  ...
  return List::create(Rcpp::Named("dense.mean") = sumdense, Rcpp::Named("xGrid") = xGrid, Rcpp::Named("beta") = betaRec, Rcpp::Named("aveN1") = aveN1, Rcpp::Named("aveN2") = aveN2, Rcpp::Named("numNoSupport") = numNoSupport, Rcpp::Named("sigma1") = sigma1Rec, Rcpp::Named("sigma2") = sigma2Rec,
