...
NumericVector makeProbsMNHA(NumericVector v) {
  int n = v.size();
...
const double root2piMNHA = sqrt(2*3.141592653589793238463);
double dNormMNHA(double x, double mu, double sigma) {
  return exp(-pow((x-mu)/sigma, 2.0)/2)/sigma/root2piMNHA;
}


//calculate the mixing probability of the mixture of two normal distributions

double makeQMNHA(double mu1, double mu2, double sigma1, double sigma2, double tau) {
...
double fASLsMNHA(double y, double mu, double lambda, double tau) {
  return log(lambda) - ((y>mu)? tau : (1.0-tau)) * std::abs(y-mu) * lambda;
}


double absTestMNHA(double a, double b) {
  return std::abs(a-b);
}

//Asymmetric Laplace density [1/lambda is reparametrized by lambda] (in the log scale) (multiple variables)

double fASLmMNHA(NumericVector y, double mu, double lambda, double tau) {
  int n = y.size();
...
double rASLsMNHA (double lambda, double tau) {
  //RNGScope scope;
...
NumericMatrix rngCppMNHA(const int N, double lambda, double tau) {
...
NumericVector rASLmMNHA (int n, double lambda, double tau) {
  RNGScope scope;
...
NumericVector testMNHA (NumericVector a, NumericVector b) {
  return (1.0-a)*b;
}

//generate random variables from multivariate normal distribution

arma::mat mvrnormArmaMNHA(int n, arma::vec mu, arma::mat sigma) {
...
IntegerVector sampleWRMNHA(int start, int end, int size, NumericVector probs) {
  int n = probs.size();
  if (n != end-start+1) {
    throw(Rcpp::exception("Something is wrong with sampleWRMNHA 1!"));
  }
  ...
  if (endpts(n) != 1.0) {
    ...
    endpts(n) = 1.0;
    //throw(Rcpp::exception("Something is wrong with sampleWRMNHA 2!"));
  }
...
int sampleOneMNHA(int start, int end, arma::colvec probs) {
  //test
  /*
...
NumericVector rnormTMNHA(int n, double mu, double sigma) {
...
arma::mat testcholMNHA(arma::mat X) {
  return arma::chol(X);
}


arma::mat testMulMNHA(arma::mat X, double a) {
  return a*X;
}


arma::mat testDivMNHA(arma::mat X, double a) {
  return 1./X;
}


arma::colvec testNormalMNHA(int n) {
  return arma::randn(n);
}


NumericVector testNormal1MNHA(int n) {
  return rnorm(n);
}
...
//'
//[[Rcpp::export]]
List bqrDPMMNHA(NumericMatrix Xr, NumericVector Yr, double tau, int nsim, NumericVector initCoef, int burn = 5000, bool plotDensity = false, int nF=10, double aD=100, double bD=100, double maxSigma=1.0/0.0, double precBeta = 0.000001, bool hasHeter = true) {
  //initialization
  ...
  for (int i=0; i<nF; i++) {
    while(q(i)<0 || q(i)>1) {
      mu1(i) = rASLsMNHA(lambda, tau);
      //Rcout<<mu1(i)<<"\n";
      mu2(i) = rASLsMNHA(lambda, tau);
      sigma1(i) = ::Rf_runif(minSigma, maxSigma);
      sigma2(i) = ::Rf_runif(minSigma, maxSigma);
      q(i) = makeQMNHA(mu1(i),mu2(i),sigma1(i),sigma2(i),tau);
      //Rcout<<mu1(i)<<"\n";
    }
  }
  NumericVector v = rbeta(nF, 1.0, D);
  v(nF-1) = 1.0;
  NumericVector probs = makeProbsMNHA(v);
  IntegerVector g = sampleWRMNHA(0,nF-1, n, probs);
  //test
  /*
  ...
  NumericMatrix keepGamma(nsim, p);
  ...
  for (int sim=0; sim<nsim; sim++) {
    ...
    for (int i=0; i<n; i++) {
    //update beta
      sdTemp(i) = sigh(i)*(h(i) * sigma1(g(i)) + (1.0 - h(i)) * sigma2(g(i)));
      normTemp(i) = (Yr(i) - sigh(i)*(h(i) * mu1(g(i)) + (1.0 - h(i)) * mu2(g(i)))) / (pow(sdTemp(i), 2.0));
    }
   ...
   /* Rcout<<gamma(0)<<"\t"<<gamma(1)<<"\n";
    Rcout<<covM(0,0)<<"\t"<<covM(0,1)<<"\n";
    Rcout<<covM(1,0)<<"\t"<<covM(1,1)<<"\n";
    for (int i=0; i<n; i++) {
      Rcout<<sdTemp(i)<<"\t";
    }*/
    ...
    if (hasHeter) {
      NumericVector mmm(n);
      NumericVector sssd(n);
      for (int i=0; i<n; i++) {
        mmm(i) = (1.0 - h(i)) * mu2(g(i)) + h(i) * mu1(g(i));
        sssd(i) = (1.0 - h(i)) * sigma2(g(i)) + h(i) * sigma1(g(i));
      }
      for (int i=1; i<p; i++) {
        att(5)++;
        cangamma = ::Rf_rnorm(gamma(i), 2*can(5));
        bool hasNeg = false;
        ...
        if (hasNeg) {
          ...
          mhRate = ::Rf_dnorm4(cangamma, 0.0, 1.0/sqrt(precBeta), 1) - ::Rf_dnorm4(gamma(i), 0.0, 1.0/sqrt(precBeta), 1);
          for (int j=0; j<n; j++) {
            if (cansigh(j)<0.0) {
              throw(Rcpp::exception("Something is wrong with rejection sampling for gamma!"));
            }
            mhRate += ::Rf_dnorm4(resids(j), mmm(j)*cansigh(j), sssd(j)*cansigh(j), 1) - ::Rf_dnorm4(resids(j), mmm(j)*sigh(j), sssd(j)*sigh(j), 1);
          }
        }
        if (!(mhRate!=mhRate)) {
    ...
    for (int i=0; i<n; i++) {
      if (h(i) == 0) {
        for (int j=0; j<nF; j++) {
          lll(j) = log(1.0-q(j)) + ::Rf_dnorm4(resids(i), mu2(j)*sigh(i), sigma2(j)*sigh(i), 1) + log(probs(j));
        }
      }
      if (h(i) == 1) {
        for (int j=0; j<nF; j++) {
          lll(j) = log(q(j)) + ::Rf_dnorm4(resids(i), mu1(j)*sigh(i), sigma1(j)*sigh(i), 1) + log(probs(j));
        }
      }
      g(i) = sampleOneMNHA(0,nF-1,arma::exp(lll-arma::max(lll)));
      /*if (i==1 && g(i)>1000) {
        Rcout<<"g(i)="<<g(i)<<"\t"<<arma::exp(lll-arma::max(lll))<<"\n";
        for (int j=0; j<nF; j++) {
          Rcout<<lll(j)<<"\n";
        }
    ...
    for (int i=0; i<n; i++) {
      //Rcout<<i<<"\t"<<g(i)<<"\t";
      p1 = q(g(i)) * ::Rf_dnorm4(resids(i), mu1(g(i))*sigh(i), sigma1(g(i))*sigh(i), 0);
      p0 = (1.0 - q(g(i))) * ::Rf_dnorm4(resids(i), mu2(g(i))*sigh(i), sigma2(g(i))*sigh(i), 0);
    ...
    for (int l=0; l<nF; l++) {
      ...
      canq = makeQMNHA(canmu1, mu2(l), sigma1(l), sigma2(l), tau);
      if (canq>0 && canq<1) {
        mhRate = fASLsMNHA(canmu1, 0, lambda, tau) - fASLsMNHA(mu1(l), 0, lambda, tau);
        numSelect = 0;
        for (int i=0; i<n; i++) {
          if (g(i) == l) {
            if (h(i) == 1) {
        ...
        if (numSelect > 0) {
          for (int j=0; j<numSelect; j++) {
            mhRate += ::Rf_dnorm4(resids(indSelect(j)), canmu1*sigh(indSelect(j)), sigma1(l)*sigh(indSelect(j)), 1) - ::Rf_dnorm4(resids(indSelect(j)), mu1(l)*sigh(indSelect(j)), sigma1(l)*sigh(indSelect(j)), 1);
          }
        }
        if (::Rf_runif(0,1) < exp(mhRate)) {
          if (probs(l)>0.2) {
      ...
      canq = makeQMNHA(mu1(l), canmu2, sigma1(l), sigma2(l), tau);
      if (canq>0 && canq<1) {
        mhRate = fASLsMNHA(canmu2, 0, lambda, tau) - fASLsMNHA(mu2(l), 0, lambda, tau);
        numSelect = 0;
        for (int i=0; i<n; i++) {
          if (g(i) == l) {
            if (h(i) == 0) {
        ...
        if (numSelect > 0) {
          for (int j=0; j<numSelect; j++) {
            mhRate += ::Rf_dnorm4(resids(indSelect(j)), canmu2*sigh(indSelect(j)), sigma2(l)*sigh(indSelect(j)), 1) - ::Rf_dnorm4(resids(indSelect(j)), mu2(l)*sigh(indSelect(j)), sigma2(l)*sigh(indSelect(j)), 1);
          }
        }
        if (::Rf_runif(0,1) < exp(mhRate)) {
          if (probs(l)>0.2) {
      ...
      for (int i=0; i<n; i++) {
        r(i) = resids(i) - (h(i) * mu1(g(i)) + (1.0-h(i))*mu2(g(i))) * sigh(i);
      }
      ...
      if (cansigma1>minSigma && cansigma1<maxSigma) {
        canq = makeQMNHA(mu1(l), mu2(l), cansigma1, sigma2(l), tau);
        if (canq>0 && canq<1) {
          mhRate = 0.0;
          numSelect = 0;
          for (int i=0; i<n; i++) {
          ...
          if (numSelect > 0) {
            for (int j=0; j<numSelect; j++) {
              mhRate += ::Rf_dnorm4(r(indSelect(j)), 0, cansigma1*sigh(indSelect(j)), 1) - ::Rf_dnorm4(r(indSelect(j)), 0, sigma1(l)*sigh(indSelect(j)), 1);
            }
          }
          if (::Rf_runif(0,1) < exp(mhRate)) {
            if (probs(l) > 0.2) {
      ...
      if (cansigma2>minSigma && cansigma2<maxSigma) {
        canq = makeQMNHA(mu1(l), mu2(l), sigma1(l), cansigma2, tau);
        if (canq>0 && canq<1) {
          mhRate = 0.0;
          numSelect = 0;
          for (int i=0; i<n; i++) {
          ...
          if (numSelect > 0) {
            for (int j=0; j<numSelect; j++) {
              mhRate += ::Rf_dnorm4(r(indSelect(j)), 0, cansigma2*sigh(indSelect(j)), 1) - ::Rf_dnorm4(r(indSelect(j)), 0, sigma2(l)*sigh(indSelect(j)), 1);
            }
          }
          if (::Rf_runif(0,1) < exp(mhRate)) {
            if (probs(l) > 0.2) {
    ...
    keepGamma.row(sim) = as<NumericVector>(wrap(gamma));
    keepMu1.row(sim) = mu1;
    keepMu2.row(sim) = mu2;
  ...
  return List::create(Rcpp::Named("nClusterT")=keepNClusterT, Rcpp::Named("nCluster")=keepNCluster, Rcpp::Named("rejRateSigma2") = rejRateSigma2, Rcpp::Named("rejRateSigma1") = rejRateSigma1, Rcpp::Named("rejRateMu2") = rejRateMu2, Rcpp::Named("rejRateMu1") = rejRateMu1, Rcpp::Named("beta") = keepBeta, Rcpp::Named("gamma") = keepGamma, Rcpp::Named("truncProb")=truncProb, Rcpp::Named("xGrid")=xGrid, Rcpp::Named("dense.mean")=sumdense, Rcpp::Named("dense.var")=sumdense2, Rcpp::Named("xGrid")=xGrid, Rcpp::Named("maxSigma")=maxSigma, Rcpp::Named("q")=q, Rcpp::Named("mu1")=keepMu1, Rcpp::Named("mu2")=keepMu2, Rcpp::Named("sigma1")=keepSigma1, Rcpp::Named("sigma2")=keepSigma2, Rcpp::Named("probs")=probs);
}
