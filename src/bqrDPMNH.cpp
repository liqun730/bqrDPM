...
double sampleQuantileNH(NumericVector array, double p) {
...
void insertSortNH(int k, NumericVector values, IntegerVector ind, int n, int left){
  //insert k-th value (we know it is new) into vector values keeping values sorted
  //left the mid calculated in findDupNH1
  //this function is used together with findDupNH1 to do the unique sort
...
void testMNH (NumericVector values){
  values(0) = 0;
}

int findDupNH(NumericVector values1, NumericVector values2, double value1, double value2, int n){
...
int findDupNH1(NumericVector values, IntegerVector ind, int k, int n){
...
NumericMatrix calcByClusterNH(NumericVector Yr, NumericVector sigma2, NumericVector mu, IntegerVector s) {
...
double calcQNormalMixNH(NumericVector prob, NumericVector sigma2Star, NumericVector muStar, double p, int n) {
...
//'
//[[Rcpp::export]]
List bqrDPMNH (NumericMatrix Xr, NumericVector Yr, double p, int nsim, NumericVector initCoef, bool sampleQ = true, int burn = 5000, bool plotDensity = false, bool hasHeter = true) {
 ...
 // NumericMatrix sigma2Rec(nsim, size);
  ...
  n = 1;
  for (i=1; i<size; i++){
    s(i) = findDupNH(sigma2Star, muStar, sigma2(i), mu(i), n);
    if (s(i) == -1){
  ...
  for (sim=0; sim<nsim; sim++){
    ...
    for (i=0; i<size; i++) {
      ...
      normTemp(i) = (Yr(i) - mu(i)*sigh(i)) / varTemp(i);
    }
    ...
    tempm = calcByClusterNH(resid, sigma2, mu, s);
    for (j=0; j<n; j++) {
    ...
    for (i=0; i<size; i++) {
      ...
      if (temp <= probCont){
        ...
        int ind = findDupNH(sigma2Star, muStar, sigma2(i), mu(i), n);
        if (ind == -1) {
          sigma2Star(n) = sigma2(i);
          ////Rcout<<"flag1!";
          clusterSize(n) = 1;
    ...
    double intercept;
    if (sampleQ) {
      intercept = sampleQuantileNH(resid, p);
    }
    else {
      intercept = calcQNormalMixNH(prob, sigma2Star, muStar, p, n);
    }
    //debug
    /*
    ...
    */
    ...
    betaRec(sim, 0) = beta(0) + intercept * gamma(0);
    for (i=1; i<ncov; i++) {
