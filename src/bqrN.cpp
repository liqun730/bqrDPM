...
//log(error density)
double LLerrorN (double x, double p, double qc) {
  return ::Rf_dnorm4(x, -qc, 1, 1);
}

//log-likelihood of beta
double LLbetaN (double beta, double sd, double p, NumericVector resid, double qc) {
  double result = 0.0;
  for (int i=0; i<resid.size(); i++) {
    result += LLerrorN(resid(i), p, qc);
  }
...
//'
//[[Rcpp::export]]
List bqrN (NumericMatrix Xr, NumericVector Yr, double p, int nsim, NumericVector initCoef, int burn = 5000) {
  ...
  for (sim=0; sim<nsim; sim++){
    ...
    for (int k=0; k<ncov; k++){
      ...
      mhRate = LLbetaN(canbeta, sdDiag(k), p, canResid, qc) - LLbetaN(beta(k), sdDiag(k), p, resid, qc);
      if (!(mhRate!=mhRate)) {
